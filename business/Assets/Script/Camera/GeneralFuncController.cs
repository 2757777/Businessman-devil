...
public class GeneralFuncController : MonoBehaviour
{
    ...
    private static float SCROLL_END_LEFT = -4f; // 左側への移動制限(これ以上左には進まない)
    private static float SCROLL_END_RIGHT = 4; // 右側への移動制限(これ以上右には進まない)
    private static float SCROLL_DISTANCE_CORRECTION = 0.9f; // スクロール距離の調整
    private static float SCROLL_END_Up = 3f; // 上側への移動制限
    private static float SCROLL_END_Down = -3f; // 上側への移動制限
        ...
        if (Input.GetMouseButton(0))
        {
            ...
            else
            {
              // TownPanel.SetActive(false);
                ...
                scrollStartFlg = true;
                if (scrollStartPos.x == 0.0f && scrollStartPos.y==0)
                {
                    // スクロール開始位置を取得
                ...
                else
                {
                    ...
                    {
                        // 直前のタッチ位置との差を取得する
                        float diffPosX = SCROLL_DISTANCE_CORRECTION * (touchMovePos.x - scrollStartPos.x);
                        float diffPosY = SCROLL_DISTANCE_CORRECTION * (touchMovePos.y - scrollStartPos.y);
                        Vector2 pos = this.transform.position;
                        pos.x -= diffPosX;
                        pos.y -= diffPosY;
                        // スクロールが制限を超過する場合、処理を止める
                        if (pos.x > SCROLL_END_RIGHT || pos.x < SCROLL_END_LEFT||pos.y > SCROLL_END_Up || pos.y < SCROLL_END_Down)
                        {
                            return;
                        }
